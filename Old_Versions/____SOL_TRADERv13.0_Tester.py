...
"""                     --------- Notes: v13.0 ----------
* program checks when opening for open positon
* 
"""
...
slow_period = 15
signal_period = 9
borrow_limit = 0.033   # Changed to be micro amount
...
def open_long_position(symbol, borrow_limit):
    ...
    price = float(ticker['price'])
     ...
     #5% stop loss (2.5% at 2x leverage) - Changed to 2%
    ...
    message = (f"* Open Long | {borrow_amount} {symbol} | at {symbol}: {price} | SL: {stop_loss}")
...
def open_short_margin_position(symbol, borrow_limit):
    ...
    message = (f"* Opened Short | {borrow_amount} {symbol} | at {symbol}: {price} | SL: {stop_loss}")
    send_notification(message), print(message)
...
def run_loop():
    ...
    if numberLong > 5:
        stop_loss = pric * 0.98 #set 2.5% stop loss for long
        message = (f'* Long Position Detected: {numberLong} SOL/USD | New Stop Loss : {stop_loss} ')
        long = True

    elif numberShort > 0.5:
        stop_loss = pric * 1.02 # set 2.5% stop loss for short
        message = (f'* Short Position Detected: {numberShort} SOL/USD | New Stop Loss : {stop_loss} ')
        short = True
        ...
        try:
            ...
            if macd[-1] > signal[-1] and macd[-2] < signal[-2]:
                ...
                if short:
                    ...
                    #Open the trade
                    stop_loss = open_long_position(symbol, borrow_limit)
            ...
            #Cross Down
                ...
                # Open a short position
                    ...
                    #Open the trade
                    stop_loss = open_short_margin_position(symbol, borrow_limit)
    ...
    # ------------------ Section that waits 4 hours & checks price nonstop - ----------------
                ...
                while datetime.datetime.now() < next_candle_time:
                    ...
                    elif short and ( testnum > stop_loss):
