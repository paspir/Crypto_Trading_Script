...
"""                     ---------Notes: 10.2.2 ----------
* TRied to fix precision error for close long method
* 
                        --------------------------
"""
...
slow_period = 15
signal_period = 9
borrow_limit = 0.1  # use 5% of available margin balance
...
def close_long_position(symbol):
    ...
    # Get the step size and precision for the symbol
    exchange_info = client.get_exchange_info()
    symbol_info = next((s for s in exchange_info['symbols'] if s['symbol'] == symbol), None)
    step_size = float(symbol_info['filters'][1]['stepSize'])
    precision = int(round(-math.log10(step_size)))

    # Round down the quantity to the nearest step size with the correct precision
    ...
    try:
        ...
        # Handle the error by rounding down the quantity to the next lowest step size
        if e.code == 51077:
            quantity = math.floor((quantity - step_size) / step_size) * step_size
            quantity = round(quantity, precision)
            client.create_margin_order(
                symbol=symbol,
                side=SIDE_SELL,
            ...
            print(f"Closed Long position for {quantity} {symbol} and repaid the 3x margin loan (with rounded quantity)")
        else:
            raise e
